...
<svelte:head>
	<title>Aula Virtual · Docente · KYC DataHub</title>
</svelte:head>
<div class="space-y-6">
					...
					<div class="flex items-start justify-between gap-2">
						<a href={`/app/classroom/clase/${classroom._id}`} class="flex items-start gap-3 min-w-0 flex-1">
							...
							<div class="min-w-0">
								...
								{/if}
								<span class={`mt-1 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${classroom.activo ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'}`}>
