									...
									{:else}
										...
										{:else}
											<!-- Sin solicitud: acá es donde el estudiante decide,
											     ...
											     F-CERT-UX-REDESIGN (2026-08-20): stepper con círculos
											     numerados + drop zone para comprobante. -->

											<!-- ── Stepper horizontal con 3 pasos ── -->
											<div class="flex items-start justify-between gap-0 mb-6 px-2">
												<!-- Paso 1 -->
												<div class="flex flex-col items-center text-center flex-1 relative">
													<div class="w-9 h-9 rounded-full bg-primary-600 dark:bg-primary-500 text-white flex items-center justify-center text-sm font-bold shadow-md shadow-primary-600/30 dark:shadow-primary-400/20 z-10">
														1
													</div>
													<p class="mt-2 text-[11px] font-semibold text-primary-700 dark:text-primary-300 leading-tight max-w-[7rem]">
														Pagás el arancel y solicitás
													</p>
												</div>
												<!-- Línea 1→2 -->
												<div class="flex-1 flex items-center mt-4 -mx-2">
													<div class="w-full h-0.5 bg-gray-300 dark:bg-dark-border"></div>
												</div>
												<!-- Paso 2 -->
												<div class="flex flex-col items-center text-center flex-1 relative">
													<div class="w-9 h-9 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-500 dark:text-gray-400 flex items-center justify-center text-sm font-bold z-10">
														2
													</div>
													<p class="mt-2 text-[11px] text-light-four dark:text-dark-four leading-tight max-w-[7rem]">
											...
											<div class="mb-4">
												<label
													for="modulo-n-{eid}"
													class="block mb-1.5 text-xs font-semibold text-light-black dark:text-dark-white"
												>
													Alcance del certificado
												</label>
												<select
													id="modulo-n-{eid}"
													class="w-full rounded-lg border border-gray-300 dark:border-dark-border bg-white dark:bg-dark-surface text-light-black dark:text-dark-white px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-shadow"
													value={hastaN}
													onchange={(ev) => {
											...
											<!-- ── Drop zone para comprobante del arancel ──
											...
											<div class="mb-4">
												<p class="block mb-1.5 text-xs font-semibold text-light-black dark:text-dark-white">
													Comprobante del pago del arancel
												</p>
												<label
													for="comprobante-nd-{eid}"
													class="group flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed
														{comprobanteNoDeudorUrl[eid]
															? 'border-green-400 dark:border-green-600 bg-green-50/50 dark:bg-green-900/10'
															: 'border-gray-300 dark:border-dark-border bg-gray-50/50 dark:bg-dark-background hover:border-primary-400 dark:hover:border-primary-600 hover:bg-primary-50/30 dark:hover:bg-primary-900/10'}
														transition-all cursor-pointer px-4 py-5"
												>
													{#if comprobanteNoDeudorSubiendo[eid]}
														...
														<p class="text-xs text-light-four dark:text-dark-four">Subiendo comprobante…</p>
													{:else if comprobanteNoDeudorUrl[eid]}
														<svg class="w-7 h-7 text-green-500 dark:text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
															<path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
														</svg>
														<p class="text-xs font-medium text-green-700 dark:text-green-400">
															Comprobante listo
														</p>
														<p class="text-[10px] text-green-600/70 dark:text-green-500/70">
															Hacé clic para reemplazar si subiste el archivo incorrecto
														</p>
													{:else}
														<svg class="w-8 h-8 text-gray-400 dark:text-gray-500 group-hover:text-primary-500 dark:group-hover:text-primary-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
															<path stroke-linecap="round" stroke-linejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
														</svg>
														<p class="text-xs text-gray-500 dark:text-gray-400 text-center">
															Seleccioná el comprobante del arancel
														</p>
														<span class="inline-flex items-center rounded-lg border border-primary-300 dark:border-primary-700 bg-primary-50 dark:bg-primary-900/20 px-3 py-1.5 text-[11px] font-semibold text-primary-700 dark:text-primary-300 group-hover:bg-primary-100 dark:group-hover:bg-primary-900/30 transition-colors">
															Seleccionar archivo
														</span>
											...
											{#if !elegibleNoDeudor.ok}
												<p class="mt-3 flex items-center gap-2 rounded-lg bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800/30 px-3 py-2 text-xs text-light-error dark:text-dark-error">
													<svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
														<path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
													</svg>
													<span>{elegibleNoDeudor.motivo}</span>
												</p>
											{:else}
												<p class="mt-3 flex items-center gap-2 rounded-lg bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800/30 px-3 py-2 text-xs text-green-700 dark:text-green-400">
													<CircleCheckIcon class="w-4 h-4 shrink-0 text-green-600 dark:text-green-400" />
													<span>
