...
<div class="space-y-6 md:space-y-8 px-1 sm:px-0">
	{#if loading}
		<DashboardSkeleton />
	{:else}
		<!-- 1. Tarjeta de Bienvenida y Perfil Rápido -->
		<div class="bg-slate-900 rounded-2xl shadow-xl p-5 sm:p-7 lg:p-10 text-white overflow-hidden relative">
			<!-- Decoración de fondo -->
			...
			<div class="relative z-10 flex flex-col md:flex-row items-center gap-4 sm:gap-6">
				<!-- Avatar Letra -->
				<div class="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 rounded-full bg-slate-800 border-4 border-slate-700 flex items-center justify-center text-2xl sm:text-3xl lg:text-4xl font-extrabold shadow-inner shrink-0 text-white">
				...
				<div class="text-center md:text-left flex-1">
					...
					<h2 class="text-xl sm:text-2xl lg:text-3xl font-bold mb-2 text-white break-words">¡Bienvenido, {$userStore.user?.nombre || 'Estudiante'}!</h2>
					<p class="text-slate-300 mb-4 sm:mb-5 text-sm sm:text-base font-medium">Bienvenido a tu portal académico de postgrado de la UAGRM.</p>
					
					<div class="flex flex-wrap items-center justify-center md:justify-start gap-3 text-sm font-medium">
						<div class="flex items-center gap-2 bg-slate-800 px-4 py-2 rounded-xl border border-slate-700 shadow-sm text-slate-200 max-w-full">
							...
							<span class="break-words">Registro: {$userStore.user?.registro || $userStore.user?.codigo_registro || $userStore.user?.username || 'N/A'}</span>
						</div>
						<div class="flex items-center gap-2 bg-slate-800 px-4 py-2 rounded-xl border border-slate-700 shadow-sm text-slate-200 max-w-full">
							...
							<span class="break-words">{$userStore.user?.email || 'No registrado'}</span>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
			<Card class="flex items-center gap-4 sm:gap-5 p-5 sm:p-6 hover:shadow-md transition-shadow">
				<div class="p-3 sm:p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl text-blue-600 dark:text-blue-400 shrink-0">
				...
				<div>
					...
					<p class="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white mt-1">{enrollments.length}</p>
				</div>
			</Card>
			
			<Card class="flex items-center gap-4 sm:gap-5 p-5 sm:p-6 hover:shadow-md transition-shadow">
				<div class="p-3 sm:p-4 bg-green-50 dark:bg-green-900/20 rounded-2xl text-green-600 dark:text-green-400 shrink-0">
				...
				<div>
					...
					<p class="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white mt-1">{cursosActivos}</p>
				</div>
			</Card>
			
			<Card class="flex items-center gap-4 sm:gap-5 p-5 sm:p-6 hover:shadow-md transition-shadow sm:col-span-2 xl:col-span-1">
				<div class="p-3 sm:p-4 bg-red-50 dark:bg-red-900/20 rounded-2xl text-red-600 dark:text-red-400 shrink-0">
				...
				<div>
					...
					<p class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mt-1 break-words">{formatCurrency(totalDeuda)}</p>
				</div>
			</Card>
		</div>
					...
					{#each enrollments as enr}
						<Card class="flex flex-col justify-between hover:shadow-xl transition-all duration-300 border-t-4 border-t-blue-600">
							<div>
								...
								<h3 class="text-base sm:text-lg font-bold text-gray-900 dark:text-white leading-snug break-words line-clamp-3">
							...
							<div class="mt-6">
								<a href={`/app/enrollments`} class="flex items-center justify-center gap-2 w-full text-center bg-blue-600 hover:bg-blue-700 text-white py-2.5 sm:py-3 rounded-xl text-sm font-bold transition-colors shadow-sm">
